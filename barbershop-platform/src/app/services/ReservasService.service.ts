...
export class ReservasService {
    ...
    try {
      ...
      if (!snapshot.empty) {
        return {
          ok: false,
          mensaje:
            'Ya tienes una reserva confirmada para esta fecha. Si necesitas servicios adicionales, por favor consulta con tu barbero.',
        };
      }
  ...
  async marcarHoraComoNoDisponible(
    ...
    const ref = doc(
      this.firestore,
      `disponibilidadBarberos/${barberoId}/fechas/${fecha}`,
    );
    ...
    const horasOriginal = data['horas'] as {
      hora: string;
      disponible: boolean;
    }[];
    const horaInicio = this.convertirHoraATime(horaObjetivo);
    const horasOrdenadas = [...horasOriginal].sort(
      (a, b) =>
        this.convertirHoraATime(a.hora).getTime() -
        this.convertirHoraATime(b.hora).getTime(),
    );
