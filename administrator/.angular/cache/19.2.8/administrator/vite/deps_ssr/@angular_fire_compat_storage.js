...
function fromTask(task) {
  return new import_rxjs.Observable((subscriber) => {
    ...
    (0, import_operators.debounceTime)(0)
  );
}
function createUploadTask(task) {
  ...
  return {
    ...
    percentageChanges: () => inner$.pipe((0, import_operators.map)((s) => s.bytesTransferred / s.totalBytes * 100))
  };
}
function createStorageRef(ref2, injector) {
  return {
    getDownloadURL: () => (0, import_rxjs.of)(void 0).pipe((0, import_operators.observeOn)(injector.get(ɵAngularFireSchedulers).outsideAngular), (0, import_operators.switchMap)(() => ref2.getDownloadURL()), pendingUntilEvent(injector)),
    getMetadata: () => (0, import_rxjs.of)(void 0).pipe((0, import_operators.observeOn)(injector.get(ɵAngularFireSchedulers).outsideAngular), (0, import_operators.switchMap)(() => ref2.getMetadata()), pendingUntilEvent(injector)),
    delete: () => (0, import_rxjs.from)(ref2.delete()),
    child: (path) => createStorageRef(ref2.child(path), injector),
    updateMetadata: (meta) => (0, import_rxjs.from)(ref2.updateMetadata(meta)),
    put: (data, metadata) => {
    ...
    list: (options) => (0, import_rxjs.from)(ref2.list(options)),
    listAll: () => (0, import_rxjs.from)(ref2.listAll())
  };
}
...
var AngularFireStorage = class _AngularFireStorage {
  ...
  constructor(options, name3, storageBucket, platformId, zone, schedulers, maxUploadRetryTime, maxOperationRetryTime, _useEmulator, _appCheckInstances) {
  ...
  upload(path, data, metadata) {
    ...
    const ref2 = createStorageRef(storageRef, this.injector);
    return ref2.put(data, metadata);
  }
  static ɵfac = function AngularFireStorage_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _AngularFireStorage)(ɵɵinject(FIREBASE_OPTIONS), ɵɵinject(FIREBASE_APP_NAME, 8), ɵɵinject(BUCKET, 8), ɵɵinject(PLATFORM_ID), ɵɵinject(NgZone), ɵɵinject(ɵAngularFireSchedulers), ɵɵinject(MAX_UPLOAD_RETRY_TIME, 8), ɵɵinject(MAX_OPERATION_RETRY_TIME, 8), ɵɵinject(USE_EMULATOR, 8), ɵɵinject(AppCheckInstances, 8));
  };
...
var GetDownloadURLPipe = class _GetDownloadURLPipe {
  ...
  transform(path) {
    if (path !== this.path) {
      ...
      this.downloadUrl$ = existing ? (0, import_rxjs.of)(existing) : this.storage.ref(path).getDownloadURL().pipe((0, import_operators.tap)((it) => this.state?.set(key, it)));
    }
  ...
  static ɵfac = function GetDownloadURLPipe_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _GetDownloadURLPipe)(ɵɵdirectiveInject(AngularFireStorage, 16), ɵɵdirectiveInject(ChangeDetectorRef, 16), ɵɵdirectiveInject(TransferState, 24));
  };
...
var AngularFireStorageModule = class _AngularFireStorageModule {
  ...
  static ɵinj = ɵɵdefineInjector({
    ...
    imports: [GetDownloadURLPipeModule, GetDownloadURLPipeModule]
  });
};
(() => {
